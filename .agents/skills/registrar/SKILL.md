...
Analise o pedido do usuário e decida para onde a informação deve ir:
- **Lição** (erros, acertos, campanhas) → `c:\Users\alber\OneDrive - PALHARES\30-39 ESTÚDIO\39 Agente de Gestão\Memoria\02_Licoes.md`
- **Decisão** (preços, políticas, fornecedores, manuais) → `c:\Users\alber\OneDrive - PALHARES\30-39 ESTÚDIO\39 Agente de Gestão\Memoria\01_Decisoes.md`
- **Pendência / Aguardando / Resolvido** → `c:\Users\alber\OneDrive - PALHARES\30-39 ESTÚDIO\39 Agente de Gestão\Memoria\04_Pessoas_Pendencias.md`
- **Projeto** (mudança de status, novo projeto) → `c:\Users\alber\OneDrive - PALHARES\30-39 ESTÚDIO\39 Agente de Gestão\Memoria\03_Projetos.md`
...
- **Lições:** Sempre inicie com `- **[YYYY-MM-DD]** **[CATEGORIA]** [Texto]`. Categorias comuns: `[COMERCIAL]`, `[DESIGN]`, `[OPERAÇÃO]`, `[CONTEÚDO]`.
...
- **Projetos:** Inicie com `- **[STATUS]** **[Nome do Projeto]**...` (Status como `[BRIEFING]`, `[CAPTAÇÃO]`, `[EDIÇÃO]`).

## 3. Fluxo de Ação Silenciosa
1. Leia o arquivo destino.
2. Identifique a seção/cabeçalho onde o item se encaixa melhor.
3. Use a ferramenta `multi_replace_file_content` (ou `replace_file_content`) para injetar a nova linha sem modificar o resto.
